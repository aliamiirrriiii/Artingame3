...
![Pistol, SMG, sniper and rifle in game](docs/screenshot-weapons.png)
...
Eight of the twelve weapons are built by a **gunsmith**
...
Three more — the pistol, the SMG and the sniper — are downloaded models from
the Steel Tide / Quaternius pack (CC0), reached through a GitHub mirror because
quaternius.com and poly.pizza are unreachable from the build sandbox. They have
better silhouettes than parametric geometry gets to: an MP5 that is
unmistakably an MP5. They also ship no textures whatsoever — flat material
colours, and meshes carrying POSITION and NORMAL only, with no UVs for a
texture to map to — so `src/weapons/viewmodels.js` re-finishes them on load.
UVs are generated by box projection in metres, matching the gunsmith's own
convention so the existing detail maps tile at a sane density; shading is baked
to vertex colours from surface orientation; and their materials are mapped onto
the game's finishes by name, which the pack makes easy by calling them `Metal`,
`DarkMetal`, `ActionSteel`, `Wood` and `Glass`.

Each one places itself from its own measurements — rear face on the rig origin,
centred across, top at the sight height — rather than from offsets typed in by
hand, and its muzzle is found by averaging the vertices at the far end of the
barrel. Hand-guessing those put an AWM's buttstock behind the camera on the
first attempt, so a 1.2 m rifle filled the screen.
